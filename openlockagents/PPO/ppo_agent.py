...
from openlockagents.PPO.model import DiscretePolicy
from openlockagents.PPO.model import Value
from openlockagents.PPO.core import update_params
from openlockagents.PPO.utils.replay_memory import Memory


class PPOAgent(Agent):
    def __init__(self, env, state_size, action_size, params, require_log=True):
        """
        Init PPO agent for OpenLock env
        ...
        :param params
        """
        super(PPOAgent, self).__init__("PPO", params, env)
        if require_log:
            super(PPOAgent, self).setup_subject(human=False)
        ...
        self.max_iter_num = params["num_training_iters"]
        self.optim_epochs = params["optim_epochs"]
        self.optim_value_epochs = params["optim_value_epochs"]
        self.optim_batch_size = params["optim_batch_size"]
    ...
    def run_trial_ppo(
        ...
        """
        Run a computer trial using PPO.
    ...
    def update(self, batch, i_iter):
        """
        Update the actor-critic model with PPO
        Args:
        """
        update_params(
            self.policy,
    ...
    def finish_subject(self, strategy="PPO", transfer_strategy="PPO"):
        super(PPOAgent, self).finish_subject(strategy, transfer_strategy, self)
